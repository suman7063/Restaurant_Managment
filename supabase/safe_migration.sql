-- Safe Migration Script - Preserves existing data
...
-- Update table constraints if needed
DO $$
BEGIN
    -- Update restaurant_tables status constraint if it doesn't include 'needs_reset'
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name LIKE '%restaurant_tables_status%' 
        AND check_clause LIKE '%needs_reset%'
    ) THEN
    ...
    -- Update orders status constraint if it doesn't include new statuses
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
